...
26 2023年C题第三问
      ...
      条件：numpy(1.20.1)、panadas(1.4.2)、matplotlib(3.7.1)、scipyscipy(1.10.1)、及scikit-learn(0.24.1)；
      输入：数据2.1和数据2.2文件（无需操作）
      功能：运行main函数，计算了两个数据集专家专业性指标及创新指标（可采用这两个指标进行分析），运用三个模型进行拟合并得出最优模型
      输出：两个数据集极差和成绩对比图、两个数据集专家专业性指标和作品创新型指标I.xlsx文件
